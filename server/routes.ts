...
- Empathetic but not saccharine`;
  ...
  // POST /api/interview/turn/stream - Streaming interview turn with SSE
  app.post("/api/interview/turn/stream", requireAuth, async (req, res) => {
    await handleInterviewTurnStreaming(req.user as any, req.body, res);
  });

  // Streaming interview turn handler using SSE
  async function handleInterviewTurnStreaming(
    user: { id: string },
    body: { message: string },
    res: express.Response
  ) {
    ...
    try {
      if (!user?.id) {
        sendSSE("error", { error: "Not authenticated" });
        res.end();
        return;
      }
      ...
      if (!useAnthropic || !anthropic) {
        sendSSE("error", { error: "Streaming requires Anthropic API" });
        res.end();
        return;
      }
      ...
      // Calculate afterMessageIndex for events
      const afterMessageIndex = existingMessages.length;

      // Prepare messages for Claude
      const claudeMessages: any[] = existingMessages.map(m => ({
        role: m.role as "user" | "assistant",
        content: m.content,
      }));

      // Check for testskip
      const lastUserMessage = [...existingMessages].reverse().find((t) => t.role === "user");
      const isTestSkip = lastUserMessage?.content?.toLowerCase().trim() === "testskip";
      const testSkipPrompt = isTestSkip ? "\n\nIMPORTANT OVERRIDE - TESTSKIP MODE: Generate the full plan card and interview complete markers." : "";
      ...
      while (maxIterations-- > 0) {
        ...
        for await (const event of stream) {
          if (event.type === "content_block_delta") {
            ...
            if (delta.type === "text_delta" && delta.text) {
              sendSSE("token", { text: delta.text });
              textContent += delta.text;
            } else if (delta.type === "input_json_delta" && delta.partial_json) {
              // Tool input being streamed - accumulate but don't send
            }
        ...
        for (const toolUse of toolUses) {
          ...
          if ((toolUse.name === "append_title_card" || toolUse.name === "append_section_header") && sessionToken) {
            const eventType = toolUse.name === "append_title_card" ? "chat.title_card_added" : "chat.section_header_added";
            let skipped = false;
            ...
            if (!skipped) {
              ...
              await storage.appendInterviewEvent(sessionToken, eventType, {
                render: { afterMessageIndex: eventAfterIndex },
                title: toolInput.title,
                subtitle: toolInput.subtitle,
              });
            }
          } else if (toolUse.name === "set_provided_name" && user.id) {
            ...
            if (name && name.length >= 1 && name.length <= 50 && /[a-zA-Z0-9]/.test(name)) {
              ...
              if (!existingUser?.providedName) {
                await storage.updateUser(user.id, { providedName: name });
                if (sessionToken) {
            ...
            const validOptions = options.filter((opt: any) => opt?.label?.trim() && opt?.value?.trim());
            if (validOptions.length > 0) {
              const optionsWithIds = validOptions.map((opt: any, idx: number) => ({
...
`;
  ...
  // POST /api/module/stream - Streaming module conversation with SSE
  app.post("/api/module/stream", requireAuth, async (req, res) => {
    // Set up SSE headers
    res.setHeader("Content-Type", "text/event-stream");
    res.setHeader("Cache-Control", "no-cache");
    res.setHeader("Connection", "keep-alive");
    ...
    try {
      ...
      if (!moduleNumber || moduleNumber < 1 || moduleNumber > 3) {
        sendSSE("error", { error: "Invalid module number" });
        res.end();
        return;
      }
      ...
      if (!loadResult.success) {
        sendSSE("error", { error: loadResult.error || "Context not ready", retryable: true });
        res.end();
        return;
      }
      ...
      if (isTestSkip) {
        systemPrompt += `\n\nIMPORTANT OVERRIDE - TESTSKIP MODE:\nThe user has entered "testskip". Generate plausible answers, call set_progress(95), then immediately call complete_module.\n`;
      }
      ...
      for (const turn of transcript) {
        if (turn?.role && turn?.content) {
          claudeMessages.push({ role: turn.role as "user" | "assistant", content: turn.content });
        }
      }
      if (transcript.length === 0) {
      ...
      for (const toolUse of toolUses) {
        ...
        if (toolUse.name === "append_structured_outcomes") {
          const optionsWithIds = (input.options || []).map((opt: any, idx: number) => ({
            id: opt.id || `opt_${idx}`,
            label: opt.label,
            value: opt.value,
          }));
          ...
          const summaryInput = input.summary || { insights: [], assessment: "", takeaway: "" };
          ...
          try {
            ...
            console.error("[MODULE_STREAM] Failed to mark module complete:", err);
          }
        }
      }
      ...
      console.error("[MODULE_STREAM] Error:", error.message);
      sendSSE("error", { error: error.message });
      res.end();
    }
  });
